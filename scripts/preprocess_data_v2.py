"""
Data Preprocessing Script V2 for Phishing Detection
Creates a 10,000 sample balanced dataset from raw sources
"""
...
# ============================================================
# CONFIGURATION - 10K Dataset (5K Legit + 5K Phishing)
# ============================================================
CONFIG = {
    # Legitimate email sources (Target: 5000)
    "enron_legit_count": 3000,          # From Enron.csv (diverse corporate)
    "spamassasin_legit_count": 1000,    # Ham from SpamAssasin.csv
    "llm_legit_count": 1000,            # From llm-generated/legit.csv
    
    # Phishing email sources (Target: 5000)
    "nazario_phishing_count": 1000,     # From Nazario.csv (classic phishing)
    "nigerian_phishing_count": 1000,    # From Nigerian_Fraud.csv (419 scams)
    "phishing_email_count": 1500,       # From phishing_email.csv
    "llm_phishing_count": 1000,         # From llm-generated/phishing.csv
    "human_phishing_count": 500,        # Original human phishing
...
def load_enron_emails(count):
    ...
    try:
        # Read more rows to get diverse samples
        df = pd.read_csv(enron_path, usecols=['subject', 'body', 'label'], nrows=100000)
...
def load_phishing_email_dataset(count):
    ...
    try:
        # Read more data for larger sample
        df = pd.read_csv(phishing_path, nrows=50000)
...
def main():
    ...
    print("10K Balanced Dataset for Better Generalization")
    print("=" * 60)
    
    # Load all datasets
    datasets = []
    
    # LEGITIMATE EMAILS (Target: 5000)
    print("\n" + "-" * 40)
    print("LOADING LEGITIMATE EMAILS (Target: 5000)")
    print("-" * 40)
    ...
    # PHISHING EMAILS (Target: 5000)
    print("\n" + "-" * 40)
    print("LOADING PHISHING EMAILS (Target: 5000)")
    print("-" * 40)
